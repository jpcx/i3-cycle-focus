 ...
 */
const getHTML = (fs, jsdocLoc, mappings) =>
  Object.keys(mappings).reduce((HTML, key) => {
    const localPath =
      mappings[key].local +
      (mappings[key].local !== '' ? '/' : '') +
      mappings[key].path +
 ...
 */
const tagCodeBlocksAsJS = string =>
  string.split(/(```)/g).reduce(
    (opData, chunk) => {
      if (chunk === '```') {
 ...
 * @returns {string} Formatted string.
 */
const fixReturnBrackets = string => string.replace(/→ {/g, '→ \\{')
 ...
 * @returns {string} Formatted string.
 */
const fixOptionalTags = string => string.replace(/<optional>/g, '\\<optional>')
 ...
 * @returns {string} Formatted string.
 */
const fixAsterixBullets = string => string.replace(/\* +\*/g, '* \\*')
 ...
 * @returns {string} Formatted string.
 */
const removeTopNamespace = string =>
 ...
 * @returns {string} Formatted string.
 */
const formatLowLevelHeaders = string =>
  string.replace(/^##### (.*:?)\n/gm, '__$1__\n')
 ...
 * @returns {string} Formatted string.
 */
const decreaseBulletSpaces = string => string.replace(/\* {3}/gm, '* ')
 ...
 * @returns {string} Formatted string.
 */
const formatThrowsAndReturnsTypes = string =>
  string
    .replace(
      /^(__Throws:__\n\n[\S\s]*?)(Type)(\n\n)([\S\s]*?)$(\n\n)/gm,
      '$1___$2:___$3* $4$5'
 ...
 * @returns {string} Formatted string.
 */
const escapeSubNamespaceStrikethrough = string =>
  string.replace(/~(\w+?~)/gm, '\\~$1')
 ...
 * @returns {string} Formatted string.
 */
const escapeOrCodeBars = string => string.replace(/\|\|/gm, '\\|\\|')
 ...
 * @returns {string} Formatted string.
 */
const escapeBarsWithinTablesFix = string =>
  string
    .split(/(^\| --- \|(?: --- \|)*?$\n[\s\S]+?\n\n)/gm)
    .reduce((formatted, block, i) => {
      if (i === 0) {
        return block
      } else if ((i + 1) % 2 === 0) {
        return (
          formatted +
          block.split(/\n/gm).reduce((newBlock, line) => {
 ...
 * @returns {string} Formatted string.
 */
const formatSourceCodeURLs = string =>
  string
    .split(/([\w\d]*\.js(?:doc)?\.html#line)/g)
    .reduce((formatted, block, i) => {
      if (i === 0) {
        return block
      } else if ((i + 1) % 2 === 0) {
        return (
          ...
          block
            .replace(/_/g, '/')
            .replace(/line/g, 'L')
            .replace(/\.js(doc)?\.html/g, '.js$1')
        )
      } else {
        return formatted + block
      }
    }, '')
    .split(/([\w\d]*\.js(?:doc)?\.html)/g)
    .reduce((formatted, block, i) => {
      if (i === 0) {
        return block
      } else if ((i + 1) % 2 === 0) {
        return (
          formatted +
          '/' +
          block.replace(/_/g, '/').replace(/\.js(doc)?\.html/g, '.js$1')
        )
      } else {
        return formatted + block
      }
...
/**
 * Formats the home footer to link to main repo page.
 *
 * @private
 * @param   {string} string - Search string.
 * @param   {Object} mappings - Map of local HTML locations to desired MD file names, locations, remote URLs, and version numbers.
 ...
 * @returns {string} Formatted string.
 */
const replaceModuleLinks = (string, mappings) =>
  Object.entries(mappings).reduce(
    (newString, entry) =>
      newString.replace(
        RegExp(entry[0].split('.').join('\\.'), 'g'),
        entry[1].repo +
          '/blob/' +
          entry[1].tag +
          '/' +
 ...
 * @returns {string} Formatted string.
 */
const replaceSourceCodeLinks = (string, mappings) =>
  string
    .split(/(\]\([/\w\d\-._~:?#[\]@!$&'()*\\+,;=`]*.js(?:doc)?[#L\d]*\))/g)
    .reduce((newString, chunk, i) => {
      if (i === 0) {
        return chunk
      } else if ((i + 1) % 2 === 0) {
        ...
        for (let val of Object.values(mappings)) {
          if (val.local === '') {
            return (
              newString + '](' + val.repo + '/blob/' + val.tag + chunk.slice(2)
            )
          }
        }
 ...
 */
const postProcess = (markdown, mappings) =>
  Object.keys(markdown).reduce((proc, key) => {
    ...
    proc[key] = formatHomeFooter(proc[key], mappings)
...
const writeMarkdown = info => {
  ...
  for (let key of Object.keys(markdown)) {
    fs.writeFileSync(process.cwd() + '/' + key, markdown[key], 'utf8')
  }
}
...
const parseConfig = config => {
  ...
  info.mappings = {}
  Object.values(config.packages).forEach(pack => {
    pack.jsdoc.forEach(doc => {
...
writeMarkdown(parseConfig(require(process.cwd() + '/' + process.argv[2])))
